...
def accepted_agent_action(item: dict) -> dict | None:
    ...
    action.setdefault("resolution", item.get("publish_resolution"))
    return action
...
def handle_batch(args: argparse.Namespace, repo: str, pr_number: str, batch_items: list[dict], *, run_id: str, iteration: int) -> tuple[str, str]:
    ...
    for item in batch_items:
        accepted_action = accepted_agent_action(item)
        payload = {
            "repo": repo,
            "pr_number": pr_number,
            "iteration": iteration,
        ...
        if accepted_action is None:
            ...
            validation_already_accepted = False
        else:
            action, error = accepted_action, ""
        ...
        if resolution not in {"fix", "clarify", "defer", "reject"} or not note:
        ...
        evidence_validation_commands = normalize_validation_commands(action.get("validation_commands"))
        default_validation_commands = normalize_validation_commands(args.validation_cmd)
        validation_commands = evidence_validation_commands + default_validation_commands
